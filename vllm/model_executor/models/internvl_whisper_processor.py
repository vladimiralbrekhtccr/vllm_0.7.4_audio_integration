# /data/nvme6n1p1/vladimir_workspace/oylan_a_v_t/vllm_new_test/vllm/vllm/model_executor/models/internvl_whisper_processor.py
# coding=utf-8
...
def load_audio(audio_input, audio_processor):
    import librosa
    import numpy as np

    sr = 16000

    # Load from file path or use raw audio
    if isinstance(audio_input, str):
        audio_values, _ = librosa.load(audio_input, sr=sr)
    elif isinstance(audio_input, np.ndarray):
        audio_values = audio_input
    else:
        raise TypeError(f"Unsupported audio input type: {type(audio_input)}")

    duration = len(audio_values) / sr
    print("Audio duration (computed from samples):", duration)
    print("Raw audio shape:", audio_values.shape)

    # Process audio using WhisperProcessor
    audio_process_values = audio_processor(
        audio_values, sampling_rate=sr, return_tensors="pt"
    )

    input_features = audio_process_values["input_features"]         # torch.Tensor
    audio_len_after_cnn = audio_process_values["audio_len_after_cnn"]  # int
    audio_token_num = audio_process_values["audio_token_num"]          # int

    # Debug prints
    print("Processed audio keys:", audio_process_values.keys())
    print("Input features shape:", input_features.shape)
    print("Audio length after CNN:", audio_len_after_cnn)
    print("Audio token count:", audio_token_num)
